...
func (bot *SmawkBot) ExecuteLabelCommand(update tgbotapi.Update, cmd []string) (tgbotapi.Message, error) {
	...
	if len(cmd) == 1 || len(cmd) == 2 {
		...
		votes, err := db.Query("UPDATE users SET label=? WHERE username=? AND chat_id=?",label,cmd[1],update.Message.Chat.ID)
		if err != nil {
			log.Fatal(err)
			return tgbotapi.Message{}, nil
		}
...
func (bot *SmawkBot) ExecuteWhoisCommand(update tgbotapi.Update, cmd []string) (tgbotapi.Message, error) {
	...
	if len(cmd) == 1 {
		...
		err = db.QueryRow("SELECT label FROM users WHERE username=? AND chat_id=?", cmd[1],update.Message.Chat.ID).Scan(&label)
		if err != nil {
			log.Fatal(err)
			return tgbotapi.Message{}, nil
